    ...
    public partial class AvenueVillageFormServeur : UserControl
    {
        ...
        private void refreshAvenueVill()
        {
            ...
            if (dgvAvenueVillage.RowCount > 0)
            {
                ...
                btnSearchAvenueVillage.Enabled = true;
            }
        ...
        private void AvenueVillageRecensseur_Load(object sender, EventArgs e)
        {
            ...
            if (dgvAvenueVillage.RowCount <= 0)
            {
                ...
                btnSearchAvenueVillage.Enabled = false;
            }
        }
        ...
        {
            string valueDesi = Interaction.InputBox("Veuillez saisir la désignation de l'avenue à rechercher", "Recherche Avenue", "", 100, 100);

            try
            {
                dgRecherche.DataSource = Factory1.Instance.SearchAvenueVillage(valueDesi);

                //On rend inisible certaines colonne des DataGridView aui recup les dataproperty
                //correspondant aux noms des accesseurs
                int col = 0;
                foreach (DataGridViewColumn dgvc in dgRecherche.Columns)
                {
                    if (dgvc.DataPropertyName == "Id") dgvc.Visible = false;
                    else if (dgvc.DataPropertyName == "Id_quartierLocalite") dgvc.Visible = false;
                    dgRecherche.AutoResizeColumn(col);
                    col++;
                }
            }
            catch (Exception ex)
        ...
        private void dgRecherche_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if (dgRecherche.SelectedRows.Count > 0)
                {
                    avenueVill = (AvenueVillageServeur)dgRecherche.SelectedRows[0].DataBoundItem;
                    lblIdAvenueVillage.Text = avenueVill.Id.ToString();
                    cboIdQuartierLoc.Text = Factory1.Instance.GetQuartierLocalite(avenueVill.Id_quartierLocalite).Designation;
                    txtDesignationAvenueVillage.Text = Convert.ToString(avenueVill.Designation);
                    lblIdQuartierLoc.Text = Convert.ToString(avenueVill.Id_quartierLocalite);
                }
            }
            catch (Exception) { MessageBox.Show("Erreur dans la zone d'affichage de rcherche", "Erreur d'affichage de rcherche"); }
        }
    }
}
